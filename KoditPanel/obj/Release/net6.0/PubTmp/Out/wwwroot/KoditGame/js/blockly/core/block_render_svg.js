 ...
 * @const
 */
Blockly.BlockSvg.SEP_SPACE_X = 6 * Blockly.BlockSvg.GRID_UNIT;
 ...
 * @const
 */
Blockly.BlockSvg.MIN_BLOCK_X = 32 * Blockly.BlockSvg.GRID_UNIT;
 ...
 * @const
 */
Blockly.BlockSvg.START_HAT_PATH = 'v -10 h 120 l 95,10';
Blockly.BlockSvg.START_BOTTOM_PATH = 'l -120,10 h -40';
...
Blockly.BlockSvg.NOTCH_PATH_RIGHT2 = (
  // 1️⃣ İlk notch
  'c -2,0 -3,1 -4,2 ' +
  'l -4,4 ' +
  'c -1,1 -2,2 -4,2 ' +
 ...
 * @const
 */
Blockly.BlockSvg.NOTCH_START_PADDING = 10 * Blockly.BlockSvg.GRID_UNIT;
...
Blockly.BlockSvg.prototype.renderFields_ =
  ...
  if(this.startHat_ || this.startBottom_)
    cursorX = 2.5 * cursorX;
...
Blockly.BlockSvg.prototype.renderDraw_ = function(iconWidth, inputRows) {
  ...
  if (this.getStartBottom()) 
    this.startBottom_ = true;
  
  if (this.getWhileHat()) 
    this.whileHat_ = true;
  
  if (this.getWhileBottom()) 
    this.whileBottom_ = true;

  if (this.getFuncHat()) 
    this.funcHat_ = true;
  
  if (this.getFuncBottom()) 
    this.funcBottom_ = true;
...
Blockly.BlockSvg.prototype.renderClassify_ = function() {
  ...
  if (this.outputConnection) {
    ...
    } else if (this.startBottom_) {
      shapes.push('bottom'); // c-block+hats are possible (e.x. reprter procedures)
    } else if (this.whileHat_) {
      shapes.push('whilehat'); // c-block+hats are possible (e.x. reprter procedures)
    } else if (this.whileBottom_) {
      shapes.push('whilebottom'); // c-block+hats are possible (e.x. reprter procedures)
    }  else if (this.funcHat_) {
      shapes.push('funchat'); // c-block+hats are possible (e.x. reprter procedures)
    } else if (this.funcBottom_) {
      shapes.push('funcbottom'); // c-block+hats are possible (e.x. reprter procedures)
    }  else if (this.ifHat_) {
      shapes.push('ifhat'); // c-block+hats are possible (e.x. reprter procedures)
    } else if (this.elseTag_) {
      shapes.push('elsetag'); // c-block+hats are possible (e.x. reprter procedures)
    } else if (this.ifEnd_) {
      shapes.push('ifend'); // c-block+hats are possible (e.x. reprter procedures)
...
Blockly.BlockSvg.prototype.renderDrawTop_ = function(pathObject, rightEdge) {
  ...
  if (this.squareTopLeftCorner_) {
    ...
    if(this.startBottom_ || this.whileBottom_ || this.funcBottom_ || this.ifBottom_ || this.elseTag_)
    {
  ...
  if (this.previousConnection) {
    ...
    if (this.startBottom_ || this.whileBottom_ || this.funcBottom_ || this.ifBottom_ || this.elseTag_)
      steps.push('H', Blockly.BlockSvg.NOTCH_START_PADDING + 60);
    else
    ...
    if (this.startBottom_ || this.whileBottom_ || this.funcBottom_ || this.ifBottom_ || this.elseTag_)
      this.previousConnection.setOffsetInBlock(connectionX + 60, 0);
    else
...
Blockly.BlockSvg.prototype.renderDrawBottom_ = function(pathObject, cursorY) {
  ...
  if (this.nextConnection) {
    ...
    if (this.startHat_ || this.whileHat_ || this.funcHat_ || this.ifHat_ || this.elseTag_) 
    {
      notchStart = (
        Blockly.BlockSvg.NOTCH_WIDTH +
        Blockly.BlockSvg.NOTCH_START_PADDING +
        Blockly.BlockSvg.CORNER_RADIUS + 60
      );
    }
    else
    {
    ...
    if(this.startHat_ || this.whileHat_ || this.funcHat_ || this.ifHat_ || this.elseTag_)
      steps.push(Blockly.BlockSvg.NOTCH_PATH_RIGHT2);
    else
    ...
    if(this.startHat_ || this.whileHat_ || this.funcHat_ || this.ifHat_ || this.elseTag_)
      this.nextConnection.setOffsetInBlock(connectionX + 60, cursorY);
    else
  ...
  if (this.startBottom_) {
    steps.push(Blockly.BlockSvg.START_BOTTOM_PATH);
  }
